...
const pub = u => ({ id: u.id, name: u.name, role: u.role, camp: u.camp, batch: u.batch, pic: u.pic || null,
  ...(u.role === 'vendor' ? Object.assign({ cat: u.cat, pkgs: u.pkgs, gallery: u.gallery || [], verified: !!u.verified, callPhone: u.callPhone || u.phone, loc: u.loc || null }, stats(u.id)) : {}) });
...
const routes = [
  ...
  ['GET', /^\/api\/vendors$/, c => {
    ...
    const list = Object.values(db.users).filter(v => v.role === 'vendor' && v.pkgs.length && (!cat || v.cat === cat) && (!camp || v.camp === camp)
